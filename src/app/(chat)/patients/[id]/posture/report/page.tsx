...
import { 
  ArrowLeft, Download, Activity, User, Target, RotateCw, Fingerprint, 
...
function generateInsights(session: PostureSession): Insight[] {
  ...
  return [
    ...
    { title: "İleri Baş Postürü", description: fhp > 12 ? `Baş dikey eksenden ${fhp.toFixed(1)}° ileride` : "Baş-boyun hizası sağlıklı.", riskScore: Math.round(fhpScore), color: c(fhpScore, 60), icon: User },
...
export default function PostureReportPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-100/50 w-full relative z-50">
      {/* ── Print / page CSS ─────────────────────────────────── */}
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .rpt-patient-avatar {
          ...
          background: #f1f5f9; object-fit: cover;
        ...
        .rpt-clinical-box p { font-size: 15px; color: #334155; line-height: 1.8; white-space: pre-line; }
        ...
        .rpt-disclaimer {
          margin-top: 24px; background: rgba(255,255,255,0.5); border-radius: 16px;
          padding: 20px 24px; font-size: 12px; color: #64748b; line-height: 1.7;
          border: 1px solid rgba(0,0,0,0.05); display: flex; gap: 16px; align-items: flex-start;
        }

        /* ── Print styles (optimized for high contrast, no useless backgrounds) ─────────────────────────────────────── */
        @media print {
          @page {
            size: A4 portrait; margin: 15mm;
          }
          body, .min-h-screen, .rpt-canvas { 
          ...
          /* Remove glass effects and shadows for print */
          .rpt-cover, .rpt-risk-card, .rpt-insight-card, .rpt-test-card, .rpt-clinical-box, .rpt-disclaimer {
            background: white !important;
          ...
          .rpt-cover { border-radius: 16px; padding: 32px; border: 2px solid #e2e8f0 !important; }
          .rpt-insight-card:hover { transform: none; box-shadow: none; }
          
          /* Control page breaks correctly */
          .rpt-section { break-inside: auto !important; page-break-inside: auto !important; display: block !important; }
          .rpt-insights-grid, .rpt-test-grid { break-inside: auto !important; page-break-inside: auto !important; display: block !important; }
          
          /* Prevent cards from breaking in half */
          .rpt-cover, .rpt-risk-card, .rpt-insight-card, .rpt-test-card, .rpt-clinical-box { 
            break-inside: avoid !important; 
            page-break-inside: avoid !important; 
          }
          
          /* Fallback for grids in print (some browsers hate grid in print) */
          .rpt-insights-grid, .rpt-test-grid, .rpt-risk-summary {
            display: flex !important;
            flex-wrap: wrap !important;
            gap: 16px !important;
          }
          .rpt-insight-card, .rpt-test-card { width: calc(50% - 8px) !important; margin-bottom: 16px !important; }
          .rpt-risk-card { width: calc(33.333% - 11px) !important; margin-bottom: 16px !important; }
          
          .rpt-test-snapshot-wrap { background: #f8fafc !important; }
      ...
      <div className="rpt-toolbar">
        <button className="rpt-btn rpt-btn-back" onClick={() => router.back()}>
      ...
      <div className="rpt-canvas" ref={printRef}>
        ...
        <div className="rpt-cover">
          <div className="rpt-cover-top">
            <div className="rpt-brand">
              <div className="rpt-brand-logo"><Activity size={28} /></div>
              <div>
        ...
        <div className="rpt-section">
          ...
          {patient.profile?.shortDescription && (
            <div className="bg-white/60 backdrop-blur-md border border-slate-200/60 rounded-2xl p-5 text-sm text-slate-600 leading-relaxed">
              <strong className="text-slate-900 flex items-center gap-2 mb-2"><Info size={16} className="text-indigo-500" /> Hasta Notu:</strong> 
              {patient.profile.shortDescription}
            </div>
          )}
        </div>
        ...
        {/* ═══════════════════════════════════════════════════════════════
            KLİNİK GÖRÜŞ
        ════════════════════════════════════════════════════════════════ */}
        {patient.profile?.clinicalOpinion && (
          <div className="rpt-section">
            <div className="rpt-section-title">
            ...
            <div className="rpt-clinical-box">
              <p>{patient.profile.clinicalOpinion}</p>
            </div>
          </div>
        )}
